module ArchiveTree
  module RailsExt
    module ActiveRecord
      ...
      # Named scopes that will be added to ActiveRecord::Base.
      module Scopes
        ...
        def archived_years
          where("created_at IS NOT NULL").
            group("EXTRACT(YEAR FROM created_at)").size.
            inject({}) {|years, year| years.tap{ |y| y[year.first.to_i] = year.last} }
        end # archived_years

        # For a given year, constructs a single-level hash of months using the defined +date_field+ column.
        #
        # The returned hash is a key-value-pair representing the number of records for a given month,
        ...
        #   * :month_names #=> Null, or absent will result in months represented as integer (default).
        #                     Also accepts :long and :short, depending on the desired lenght length for the month name.
        #
        # *Considerations*
        ...
        def archived_months(options = {})
          ...
          where("EXTRACT(YEAR FROM #{date_field}) = ?", options.delete(:year) || Time.now.year).
            group("EXTRACT(MONTH FROM #{date_field})").size.
            inject({}) do |months, month|
              months.tap do |months|
                value = month.first.to_i
                count = month.last
        ...
        #   * :month_names => Accepts one of two symbols :long and :short. The default will be int
        #
        # Examples context
        ...
        #
        # *Considerations*
        # This method has poor performance due to the 'linear' way in which the queries are being constructed and
        # executed.
        #
        # TODO: Optimize the queries.
