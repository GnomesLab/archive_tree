...
begin
  if File.exists?(database_yml_file)
    active_record_configuration = YAML.load_file(database_yml_file)[ENV['test']]
  ...
  raise "Something went wrong while attempting to load your database file, or while configuring ActiveRecord."
end
